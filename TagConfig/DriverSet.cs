...
namespace TagConfig
{
    public partial class DriverSet : Form
    {
        ...
        public DriverSet(DriverMetaData device, List<RegisterModule> typeList, List<DriverArgumet> args)
        {
            _device = device;
            _typeList = typeList;
            _arguments = args;
            ...
            col.DisplayMember = "ClassName";
            col.ValueMember = "DriverID";
        }
        ...
        private void Form3_Load(object sender, EventArgs e)
        {
            if (_device != null)
            {
                col.SelectedValue = _device.DriverID;
        ...
        {
            _device.DriverID = Convert.ToInt32(col.SelectedValue);
        }

        private void GetProperties(bool isnew)
        {
            var item = _typeList.Find(x => x.DriverID == _device.DriverID);
            if (item != null)
            {
                try
                {
                    ...
                    {
                        string path = Application.StartupPath + item.AssemblyName;
