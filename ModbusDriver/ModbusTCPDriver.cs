    ...
    public sealed class ModbusTCPReader : IPLCDriver, IMultiReadWrite                    //IPLCDriver : IDriver, IReaderWriter       IDriver : IDisposable
    {
                ...
                case '3':
                    {
                        ...
                        dv.Start--;
                        dv.Bit--;
                        dv.ByteOrder = ByteOrder.Network| ByteOrder.BigEndian;
                    }
                    break;
                case '4':
                    {
        ...
        public bool Connect()
        {
            ...
            try
            {
                ...
                tcpSynCl.Connect(this.ServerName, this.Port);
                return true;
            }
        ...
        private byte[] CreateWriteHeader(int id, int startAddress, ushort numData, ushort numBytes, byte function)
        {
            ...
            data[6] = (byte)this.SlaveID;          			// Slave address
            data[7] = function;				// Function code
            byte[] _adr = BitConverter.GetBytes(IPAddress.HostToNetworkOrder((short)startAddress));
            data[8] = _adr[0];				// Start address
            data[9] = _adr[1];				// Start address
        ...
        public byte[] ReadBytes(DeviceAddress address, ushort size)
        {
            ...
            if (area < 3)
                return WriteSyncData(CreateReadHeader(address.Area, address.Start * 16, (ushort)(16 * size), (byte)area));
            else
                return WriteSyncData(CreateReadHeader(address.Area, address.Start, size, (byte)area));
